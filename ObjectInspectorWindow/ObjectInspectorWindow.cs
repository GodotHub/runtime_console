...
public partial class ObjectInspectorWindow : Window
{
    ...
    */
    ...
    private void FillObjectTree()
    {
        ...
        if (_editorIcons != null)
            rootItem.SetIcon(0, GetIcon(rootNode));
        CreateNodeItem(rootNode, rootItem);
    }

    private void CreateNodeItem(Node rootNode, TreeItem parent)
    {
        ...
        foreach (var child in rootNode.GetChildren())
        {
            ...
            item.SetMetadata(0, child);
            item.SetText(0, child.Name);
            if (_editorIcons != null)
                item.SetIcon(0, GetIcon(child));
            CreateNodeItem(child, item);
        }
    }
